...
@login_required
def home():
    if current_user.role.name == 'Reporter': return redirect(url_for('risk_register'))
...
def get_risks():
    ...
    for r in risks:
        risk_data = {
            'id': r.id, 'risk_code': r.risk_code, 'title': r.title, 'description': r.description, 'risk_type': r.risk_type,
            'category': r.category, 'probability': r.probability, 'impact': r.impact, 'risk_level': r.risk_level, 
            'owner': r.owner, 'risk_location': r.risk_location, 'proactive_actions': r.proactive_actions, 
            'immediate_actions': r.immediate_actions, 'action_effectiveness': r.action_effectiveness, 'status': r.status, 
            'created_at': r.created_at.isoformat(), 'residual_risk': r.residual_risk, 'attachment_filename': r.attachment_filename, 
            'user_id': r.user_id, 'lessons_learned': r.lessons_learned, 'is_read': r.is_read, 'was_modified': r.was_modified,
...
def delete_risk(risk_id):
    risk = Risk.query.get_or_404(risk_id)
    if current_user.role.name != 'Admin' and risk.user_id != current_user.id: return jsonify({'success': False, 'message': 'غير مصرح لك بحذف هذا الخطر'}), 403
...
def delete_attachment(risk_id):
    risk = Risk.query.get_or_404(risk_id)
    if current_user.role.name != 'Admin' and risk.user_id != current_user.id: return jsonify({'success': False, 'message': 'غير مصرح لك'}), 403
    if risk.attachment_filename:
        file_path = os.path.join(app.config['UPLOAD_FOLDER'], risk.attachment_filename)
        if os.path.exists(file_path): os.remove(file_path)
...
if __name__ == '__main__':
    with app.app_context():
        ...
        # 2. إنشاء المستخدمين وربطهم بالأدوار
